...
/**
 * Migration: Create real_time_trades table (atomic via transaction).
 *
 * Live dashboard table: open trades and recently closed trades (24hr window).
 * Synchronization from `trades` is handled by a separate trigger migration.
 */
...
export async function down(queryInterface: QueryInterface): Promise<void> {
  await queryInterface.sequelize.transaction(async (t: Transaction) => {
    await queryInterface.dropTable('real_time_trades', { transaction: t });
  });
}
