...
import { MetaApiConfigService } from './src/integrations/metaapi/metaapi-config.service';
import { MetaApiHttpModule } from './src/integrations/metaapi/metaapi-http.module';
...
async function testMetaApi() {
  console.log('🚀 Starting MetaApi Integration Test (Standalone - No Database)...\n');

  // Create a minimal test module without DatabaseModule
  const moduleRef = await Test.createTestingModule({
    imports: [MetaApiHttpModule],
    providers: [ProvisioningService, TradingService],
  }).compile();

  const provisioningService = moduleRef.get<ProvisioningService>(ProvisioningService);
  const tradingService = moduleRef.get<TradingService>(TradingService);
  const config = moduleRef.get<MetaApiConfigService>(MetaApiConfigService);

  console.log('✅ Services initialized successfully\n');
  console.log('📝 Configuration:');
  console.log(`   Provisioning Base URL: ${config.provisioningBaseUrl}`);
  console.log(`   Trading Base URL: ${config.tradingBaseUrl}`);
  console.log(`   Region: ${config.region}`);
  console.log(`   Account ID (static): ${config.accountId || 'Not set (will fetch dynamically)'}\n`);
